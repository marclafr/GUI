...
bool UI_Button::Draw(SDL_Texture * atlas)
{
	...
	{
	case UI_B_IDLE:
		App->render->Blit(atlas, img_idle->pos.x, img_idle->pos.y, &img_idle->atlas_rect);
		break;

	case UI_B_MOUSE_ON_TOP:
		App->render->Blit(atlas, img_mouse_on_top->pos.x, img_mouse_on_top->pos.y, &img_mouse_on_top->atlas_rect);
		break;

	case UI_B_CLICKING:
		App->render->Blit(atlas, img_clicking->pos.x, img_clicking->pos.y, &img_clicking->atlas_rect);
		break;
